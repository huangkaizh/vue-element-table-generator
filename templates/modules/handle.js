...
var methodsObj = {
  add: function(req, res, next) {
    ...
    pool.getConnection(function(err, connection) {
      /* ${`const obj = [${settings.columns.map(column => {
        const defaultValue = column.formItem && column.formItem.defaultValue ? column.formItem.defaultValue : null;
        if(['bigint', 'bit', 'decimal', 'double', 'float', 'int', 'integer', 'numeric', 'smallint','tinyint', ''].includes(column.DB_DATA_TYPE)) {
          return +(param.age || defaultValue || 0)
        }  else {
          return param.name || defaultValue || ""
        }
      }).join(',')}];`} */
      connection.query(sql.insert, obj, function(err, result) {
        if (result) {
          result = "add";
        }
  ...
  update: function(req, res, next) {
    ...
    pool.getConnection(function(err, connection) {
      /* ${`const fieldValues = [${settings.columns.map(column => {
        return `param.${column.name}`
      }).join(',')}];`} */
      connection.query(sql.update, fieldValues, function(err, result) {
        if (result.affectedRows > 0) {
          result = "update";
        } else {
          result = undefined;
  ...
  queryAll: function(req, res, next) {
    ...
    Object.keys(req.query)
      .filter(key => {
        return !["page", "size", "sort"].includes(key);
      })
      .forEach(key => {
        const fieldValue = req.query[key];
        if (fieldValue) {
          whereString += ` and name like '%${req.query[key]}%'`;
        }
      });
